...
class AstraVectorStoreComponent(LCVectorStoreComponent):
    ...
    def search_documents(self, vector_store=None) -> list[dict]:
        ...
        search_method = "similarity_search_with_score" if self.search_type in ["Similarity", "Similarity with score threshold"] else "metadata_search"
        self.log(f"Using search method: {search_method}")

        try:
            self.log(f"Calling vector_store.{search_method} with args: {search_args}")

            if search_method == "similarity_search_with_score":
                # Perform similarity search with scores
                results = vector_store.similarity_search_with_score(
                    query=search_args["query"],
                    k=search_args.get("k", 10),
                    score_threshold=search_args.get("score_threshold", 0),
                )
                # Prepare results with document and score
                data = [{"document": doc.page_content, "score": score} for doc, score in results]
            else:
                # Fallback to other search method
                docs = getattr(vector_store, search_method)(**search_args)
                data = docs_to_data(docs)

            self.log(f"Retrieved results: {len(data)}")
            self.status = data
            return data
        except Exception as e:
